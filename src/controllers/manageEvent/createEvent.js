...
export default router.post("/", authenticate, async (req, res) => {
  try {
    ...
    for (let i = 0; i < student_id.length; i++) {
      ...
      let message = {
        subject: `Meeting Link for Sri Sathya Sai Dakshata Program on ${moment
          .utc(datetime)
          .local()
          .format("LL")}`,
...
We are pleased to invite you to the upcoming meeting for the Sri Sathya Sai Dakshata Program.
