...
export default router.post("/", async (req, res) => {
  try {
    uploads(req, res, async (err) => {
      ...
      // Password field removed from registration UI; enforce default password
      // for all new registrations (existing password column is unchanged).
      const encryptedPassword = CryptoJS.AES.encrypt(
        "Sairam@123",
        process.env.TOKEN_KEY,
      ).toString();
      ...
      let student = await studentModel.create({
        ...
        role: ROLE.STUDENT,
        password: encryptedPassword,
        // Fields hidden from registration UI; enforce defaults server-side.
        gmat: "0",
        iq_level: "0",
        sk_analyticalskill: "1", // Beginner
        sk_prblmsolving: "1", // Beginner
        hckr_rnk: "Beginner",
        attitude: "0",
        resume,
        profile,
      ...
      let message = {
        subject: `Confirmation of Interest in Data Engineer Course`,
        text: `Dear ${student.name},
We are pleased to acknowledge your registration for the Data Engineer Course offered by the Sri Sathya Sai Dakshata Program.
...
Best regards,
Program Coordinator
Sri Sathya Sai Dakshata Program
🌐 [www.dakshata.ai](http://www.dakshata.ai)
📞 9052372023`,
      };
